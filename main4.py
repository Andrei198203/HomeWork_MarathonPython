...
def on_seat_right_click(x, y): #ДЗ2
    seat = get_seat(x, y)
    if seat:
        seats[seat] = False
...
def double_on_seat_click(x, y): #ментор порадив
    seat = get_seat(x, y)
    if seat:
        seats[seat] = False
        draw_seat(*seat, color="steel blue")    
        write_free_seats() 
        write_sold_seats()   
...
main_screen.onclick(on_seat_click, btn = 3) 
